...
error_reporting(0);
 ...
 $con=$dbi->connect();
//Nairobi
$nbpep=$dbi->sum_facility_pep($con,25436);
$nbnonpep=$dbi->sum_facility_nonpep($con,25436);
$nbreexposed=$dbi->sum_facility_reexposed($con,25436);


//Mombasa
$mbpep=$dbi->sum_facility_pep($con,54321);
$mbnonpep=$dbi->sum_facility_nonpep($con,54321);
$mbreexposed=$dbi->sum_facility_reexposed($con,54321);

//Isiolo
$ispep=$dbi->sum_facility_pep($con,21000);
$isnonpep=$dbi->sum_facility_nonpep($con,21000);
$isreexposed=$dbi->sum_facility_reexposed($con,21000);

//Nairobi
$nb2pep=$dbi->sum_facility_pep($con,18344);
$nb2nonpep=$dbi->sum_facility_nonpep($con,18344);
$nb2reexposed=$dbi->sum_facility_reexposed($con,18344);
...
${demo.css}
       ...
       <input type="hidden" name="nbnonpep" class="nbnonpep" placeholder="Search..." value="<?php echo $nbnonpep; ?>" >	
        <input type="hidden" name="nbreexposed" class="nbreexposed" placeholder="Search..." value="<?php echo $nbreexposed; ?>" >
         ...
         <input type="hidden" name="nb2pep" class="nb2pep" placeholder="Search..." value="<?php echo $nb2pep; ?>" >		
       <input type="hidden" name="nb2nonpep" class="nb2nonpep" placeholder="Search..." value="<?php echo $nb2nonpep; ?>" >	
        <input type="hidden" name="nb2reexposed" class="nb2reexposed" placeholder="Search..." value="<?php echo $nb2reexposed; ?>" >			
...
// Create the cha
	...
	var nb2pep= $(".nb2pep").val();
	var clients_nb2pep=parseInt(nb2pep);
	var nb2nonpep= $(".nb2nonpep").val();
	var clients_nb2nonpep=parseInt(nb2nonpep);
	var nb2reexposed= $(".nb2reexposed").val();
	var clients_nb2reexposed=parseInt(nb2reexposed);
		//alert (tfacilities);
	var mbpep= $(".mbpep").val();
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Clients Status per Facility'
        },
        subtitle: {
        ...
        xAxis: {
            categories: [
                'Shwari Medical Clinic',
                'Afya House Dispensary',
				'77 Isiolo Medical Clinic',
                'AAR Medical Clinic',
                'Siaya County Hospital',
                'Kiambu Sub county Hospital',
        ...
        tooltip: {
            ...
            pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
                '<td style="padding:"><b>{point.y:.f} </b></td></tr>',
            footerFormat: '</table>',
            shared: true,
            useHTML: true
        },
       ...
       series: [{
            name: 'PEP',
            data: [clients_nbpep, clients_nb2pep, clients_mbpep, clients_ispep]

        }, {
            name: 'Non PEP',
            data: [clients_nbnonpep, clients_nb2nonpep,  clients_mbnonpep, clients_isnonpep]

        }, {
            name: 'Reexposed',
             data: [clients_nbreexposed, clients_nb2reexposed, clients_mbreexposed, clients_isreexposed]
                    ...
                    <div class="row">
                        ...
                        <div class="box">
                            <div class="box-header">
                                <h3 class="box-title">Client's Status per Facility</h3>
                            </div><!-- /.box-header -->
                            <div class="box-body">
<!--                                <input type="datetime" name="date_from" id="date_from" placeholder="Date  From" class="date_from "/>
						 ...
						 $p_rs=$dbi->getReportList($con);
    	                  while($p_row=mysql_fetch_array($p_rs)) {
                                       
                                                    $p_id=$p_row['p_id'];
													$mfl_no=$p_row['mfl_no'];
                                                    $f_name=$dbi->getHospital($con,$mfl_no);
													
													 //Aggregation
                                                    $tclients=$dbi->sum_pep_clients($con,$mfl_no);
												    $tpep=$dbi->sum_pep($con,$mfl_no);
													$tnonpep=$dbi->sum_nonpep($con,$mfl_no);
													$treexposed=$dbi->sum_reexposed($con,$mfl_no);
